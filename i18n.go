...
var i18n = map[string][2]string{
	...
	"prune.usage_bad_before":      {"csm prune: invalid --before %q (want YYYY-MM-DD)", "csm prune: --before %q 형식 오류 (YYYY-MM-DD 형태여야 합니다)"},
	"prune.usage_days_and_before": {"csm prune: use either <days> or --before, not both", "csm prune: <days> 와 --before 는 함께 쓸 수 없습니다"},
	"prune.threshold_days":        {"%d days", "%d일"},
	"prune.none":                  {"no sessions older than %s (pinned sessions excluded)", "%s 이전 세션 없음 (핀 제외)"},
	"prune.preview_header":        {"%d sessions older than %s will be %s", "%d개 세션이 %s 이전 — %s 됩니다"},
